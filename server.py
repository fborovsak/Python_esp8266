...
def serve(reader, writer):
    try:
        file, method = decode_path((yield from reader.read()))
        fileNoExt = stripExtension(file)
        if exists(file):
            ...
            while buffer != b'':
                ...
                buffer = f.read(512)
            f.close()
        elif exists(fileNoExt + ".py"):
            page = __import__(fileNoExt)
            if (hasattr(page, 'serve')):
                mime_type, cacheable = get_mime_type(".html")
                yield from writer.awrite("HTTP/1.0 200 OK\r\n")
                yield from writer.awrite("Content-Type: {}\r\n".format(mime_type))
                if cacheable:
                    yield from writer.awrite("Cache-Control: max-age=86400\r\n")
                ...
                content = page.serve(method)
                yield from writer.awrite(content)
        else:
